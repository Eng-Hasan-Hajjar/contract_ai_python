"""
╔══════════════════════════════════════════════════════════════════╗
║      نظام تحليل العقود التجارية  –  الإصدار 3.0               ║
║      مشروع تخرج  |  تحليل بقواعد ذكية بدون AI                 ║
║      خط عربي كبير  |  كشف العناوين  |  تقييم قوة العقد        ║
╚══════════════════════════════════════════════════════════════════╝
  pip install customtkinter pdfplumber python-bidi arabic-reshaper
  (anthropic و reportlab اختياريان)
"""
...
# ── تصحيح النص العربي المعكوس من PDF ────────────────────────
try:
...
def _is_reversed(s: str) -> bool:
    """يكتشف إذا كان السطر معكوساً من PDF Visual-RTL"""
    if not s: return False
    # ينتهي برقم+نقطة  (عنوان معكوس مثل 'دقعلا ةدم3.')
    if re.search(r'\d[\.\:]$', s): return True
    # يبدأ بعلامة ترقيم (النقطة أو النقطتان جاءت في النهاية الأصلية)
    if s[0] in '.،؛:؟!': return True
    # إذا عكسناه يبدأ برقم+نقطة (عنوان بند)
    if re.match(r'^\d+[\.\-\)]\s', s[::-1]): return True
    # يحتوي على عربي والحرف الأول عربي
...
def _fix_line(line: str) -> str:
    """
    يُصحّح سطراً واحداً مستخرجاً من PDF عربي Visual-RTL.
    الحل: عكس الأحرف + إصلاح الأرقام.
    ...
    # عكس الأحرف
    rev = s[::-1]
    # إصلاح رقم البند:  '.9القانون' → '9. القانون'
    rev = re.sub(r'^\.(\d+)', r'\1.', rev)
    rev = re.sub(r'^(\d+\.)(\S)', r'\1 \2', rev)
    # إعادة عكس الأرقام المستقلة (السنوات والتواريخ)
...
def fix_arabic(text: str) -> str:
    """
    يُصحّح نصاً كاملاً مستخرجاً من PDF عربي معكوس.
    يستخدم bidi إن كانت مثبّتة، وإلا يعكس الأحرف سطراً سطراً.
    """
    ...
    if ARABIC_FIX:
        try:
            ...
            for ln in text.split("\n"):
                out.append(get_display(reshape(ln)) if ln.strip() else ln)
            return "\n".join(out)
        except Exception:
            pass
...
class State:
    ...
    def __init__(self):
        self.reset()
        self.current_user   = None   # dict المستخدم الحالي بعد الدخول
...
def extract_headings(path: str):
    """
    يستخرج العناوين من PDF مع تصحيح النص العربي المعكوس.
    ─ لا يعود أبداً لـ _demo_headings() إلا إذا كانت pdfplumber غير مثبّتة
    ─ يطبع سبب الخطأ بدلاً من إخفائه
    """
    if not PDF_OK:
        return _demo_text(), _demo_headings()
    ...
    try:
        with pdfplumber.open(path) as pdf:
            ...
            for pnum, page in enumerate(pdf.pages, 1):
                # ── 1. استخراج النص الكامل وتصحيحه ───────────────
                try:
                    raw_page   = page.extract_text() or ""
                    fixed_page = "\n".join(_fix_line(ln) for ln in raw_page.split("\n"))
                    all_text.append(fixed_page)
                except Exception as e:
                    error_log.append(f"ص{pnum} extract_text: {e}")
                    all_text.append("")
                    fixed_page = ""

                # ── 2. اكتشاف bold من بيانات الكلمات ──────────────
                bold_titles: set[str] = set()
                try:
                    words = page.extract_words(extra_attrs=["fontname","size"])
                    ld = {}
                    for w in words:
                        y = round(float(w.get("top",0)), 0)
                        ld.setdefault(y, []).append(w)
                    for ws in ld.values():
                        fns  = [x.get("fontname","") for x in ws]
                        szs  = [float(x.get("size",10)) for x in ws]
                        bold = any("Bold" in f or "bold" in f or
                                   "Heavy" in f or "Black" in f for f in fns)
                        large = (sum(szs)/len(szs) if szs else 10) >= 12
                        if bold or large:
                            # نصوص هذا السطر
                            raw_line = " ".join(x["text"] for x in ws).strip()
                            fixed_ln = _fix_line(raw_line)
                            bold_titles.add(fixed_ln[:80])
                            bold_titles.add(raw_line[:80])
                except Exception as e:
                    error_log.append(f"ص{pnum} extract_words: {e}")

                # ── 3. اكتشاف العناوين من النص المُصحَّح ───────────
                for fl in fixed_page.split("\n"):
                    fl_s = fl.strip()
                    if fl_s and is_heading(fl_s):
                        is_bold = (fl_s[:80] in bold_titles or
                                   len(fl_s) < 60)   # العناوين عادةً قصيرة
                        heads.append({
                            "title": fl_s[:100],
                            "page":  pnum,
                            "bold":  is_bold,
        ...
        error_log.append(f"فتح الملف: {e}")

    # ── 4. إزالة مكررات مع الحفاظ على الترتيب ─────────────────
    seen, unique = set(), []
    for h in heads:
        k = re.sub(r'\s+', ' ', h["title"])[:55]
    ...
    # سجّل الأخطاء إن وجدت (للتشخيص)
    if error_log:
        print(f"[extract_headings] تحذيرات: {'; '.join(error_log)}")

    # ── 5. لا نعود للـ demo أبداً — نُعيد ما وجدناه فعلاً ────
    return full_text, unique

def _demo_headings():
    return [
        {"title":"المادة الأولى: أطراف العقد وبياناتهم",              "page":1,"bold":True},
        {"title":"المادة الثانية: موضوع العقد ونطاق التطبيق",          "page":1,"bold":True},
        {"title":"المادة الثالثة: مدة سريان العقد وشروط التجديد",      "page":2,"bold":True},
        {"title":"المادة الرابعة: الأسعار والمقابل المالي",            "page":2,"bold":False},
        {"title":"المادة الخامسة: شروط وآليات الدفع والسداد",          "page":3,"bold":True},
        {"title":"المادة السادسة: التزامات الطرف الأول",               "page":3,"bold":True},
        {"title":"المادة السابعة: التزامات الطرف الثاني",              "page":4,"bold":True},
        {"title":"المادة الثامنة: الجزاءات والتعويضات",                "page":5,"bold":False},
        {"title":"المادة التاسعة: القوة القاهرة",                      "page":6,"bold":True},
        {"title":"المادة العاشرة: آلية حل النزاعات والتحكيم",          "page":6,"bold":False},
        {"title":"المادة الحادية عشرة: القانون الواجب التطبيق",        "page":7,"bold":True},
        {"title":"المادة الثانية عشرة: أحكام ختامية",                 "page":7,"bold":True},
    ]

def _demo_text():
    return "نص عقد توضيحي\n" + "\n".join(h["title"] for h in _demo_headings())
...
# ── كلمات شائعة تُجاهَل في المطابقة ─────────────────────────
COMMON_WORDS = {
...
def match_clause(heading: str, expected_list: list):
    """
    مطابقة بنظام النقاط:
    ─ كلمة من CLAUSE_KEYWORDS موجودة في الكلمات الخاصة للعنوان → وزن مضاعف
    ─ موجودة في النص الكامل للعنوان (بما فيه دون مسافات) → وزن عادي
    ─ يُختار البند الأعلى نقاطاً، الحد الأدنى 6 نقاط
    """
    h_raw    = heading.lower().strip()
    h_clean  = re.sub(r'^\d+[\.\-\)]\s*', '', h_raw).strip()   # بدون رقم
    h_bare   = re.sub(r'^بند\s+', '', h_clean).strip()          # بدون "بند "
    h_bare_ns = re.sub(r'\s+', '', h_bare)                      # بدون مسافات
    # كلمات خاصة (حذف الشائع)
    h_words  = {w for w in h_clean.split() if w not in COMMON_WORDS and len(w) > 2}

    best_match, best_score = None, 0
    for exp in expected_list:
        kws   = CLAUSE_KEYWORDS.get(exp, [w for w in exp.split() if len(w) > 2])
        score = 0
        for kw in kws:
            kw_l  = kw.lower()
            kw_ns = re.sub(r'\s+', '', kw_l)
            if kw_l in h_words:
                score += len(kw_l) * 2          # وزن مضاعف
            elif kw_ns in h_bare_ns:
                score += len(kw_l)              # تطابق بدون مسافات
...
def analyze_with_ai(api_key, text, contract_type, rule_res) -> dict:
    if not AI_OK or not api_key.strip().startswith("sk-"):
        return rule_res
    import json
    missing_str = "، ".join(rule_res.get("missing",[]))
    found_str   = "، ".join(rule_res.get("found",{}).keys())
    prompt = (f"أنت محامٍ. حلّل العقد ({contract_type}) وأجب بـ JSON فقط:\n"
              f"نص: {text[:2500]}\n"
              f"موجود: {found_str}\nمفقود: {missing_str}\n"
              f"درجة القواعد: {rule_res.get('score',0)}/100\n"
              f'{{"summary":"...","ai_suggestions":["..."],"legal_risks":["..."]}}')
    try:
        ...
        raw = re.sub(r'\s*```$','',raw)
        d = json.loads(raw)
        rule_res["ai_summary"]     = d.get("summary","")
        rule_res["ai_suggestions"] = d.get("ai_suggestions",[])
        rule_res["ai_used"]        = True
    except Exception as e:
        rule_res["ai_error"] = str(e)
        rule_res["ai_used"]  = False
    return rule_res
...
class SidePanel(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        ctk.CTkButton(inner, text="💾  حفظ المفتاح", height=36,
                       ...
                       font=F["sm_b"], command=self._save).pack(fill="x")
        self.key_lbl = lbl(inner,"",F["xs"],C["t3"])
        ...
        lbl(inner,"v3.0 – مشروع تخرج",F["xs"],C["t3"]).pack(side="bottom")
...
class AnalysisView(ctk.CTkFrame):
    ...
    def _extract_thread(self):
        ...
        for v,t in [(0.2,"قراءة الملف..."),(0.5,"استخراج النص..."),
                    (0.8,"تحليل الهيكل..."),(1.0,"✓  اكتملت القراءة")]:
    ...
    def _analysis_thread(self):
        ...
        if ST.api_key:
            res = analyze_with_ai(ST.api_key, ST.raw_text, ct, res)
        ST.result = res
    ...
    def _show_result(self, res, info):
        ...
        # AI ملخص
        if res.get("ai_used"):
            ai_f = ctk.CTkFrame(self.result_box, fg_color=C["bg2"], corner_radius=10,
                                border_width=1, border_color=C["purple"])
            ai_f.pack(fill="x",padx=6,pady=4)
            lbl(ai_f,"🤖  ملخص Claude AI",F["sm_b"],C["purple"]).pack(anchor="w",padx=14,pady=(10,4))
            lbl(ai_f,res.get("ai_summary",""),F["body"],C["t2"],
                wraplength=380,justify="right").pack(anchor="e",padx=14,pady=(0,10))
        ...
        if res.get("ai_suggestions"):
            self._sec("💡  اقتراحات Claude AI", C["purple"])
            for s in res["ai_suggestions"]:
                lbl(self.result_box,f"  •  {s}",F["body"],C["t2"],"w",
                    wraplength=380).pack(anchor="w",padx=16,pady=3)
...
class ReportView(ctk.CTkFrame):
    ...
    def refresh(self):
        ...
        if res.get("ai_summary"):
            self._sec("🤖  التحليل القانوني من Claude AI",C["purple"])
            lbl(self.sc,res["ai_summary"],F["body"],C["t2"],
                wraplength=750,justify="right",anchor="e").pack(anchor="e",padx=20,pady=6)
            for s in res.get("ai_suggestions",[]):
                lbl(self.sc,f"  💡  {s}",F["body"],C["t2"],"w",
                    wraplength=700).pack(anchor="w",padx=20,pady=3)
...
class App(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("نظام تحليل العقود الذكي  –  v3.0  |  مشروع تخرج")
    ...
    def _build(self):
        ...
        self.neg_v = NegotiationView(
                self.content,
                get_user=lambda: ST.current_user,
            )
    ...
    def _build_status(self):
        ...
        missing_libs = (["pdfplumber"] if not PDF_OK else []) + (["reportlab"] if not RL_OK else []) + (["python-bidi arabic-reshaper"] if not ARABIC_FIX else [])
        if missing_libs:
            lbl(si,f"⚠  تثبيت مطلوب: {', '.join(missing_libs)}  (pip install ...)",
                F["xs"],C["orange"]).pack(side="right",padx=14,pady=6)

    def _nav(self, view):
        for v in [self.home_v, self.analysis_v, self.report_v,self.history_v, self.neg_v]:
            v.pack_forget()
        if view=="home":
            ...
            self.neg_v.pack(fill="both", expand=True)
            self.neg_v.refresh()
            self.status.configure(text="إدارة التفاوض بين الأطراف")    
        elif view=="logout":
...
if __name__ == "__main__":
    ...
    print("═"*55)
    print("  نظام تحليل العقود  –  v3.0  |  مشروع تخرج")
    print("═"*55)
    print(f"  pdfplumber       : {'✓' if PDF_OK      else '✗  pip install pdfplumber'}")
    print(f"  python-bidi      : {'✓' if ARABIC_FIX  else '✗  pip install python-bidi arabic-reshaper'}")
    print(f"  anthropic        : {'✓' if AI_OK       else '✗  pip install anthropic  (اختياري)'}")
    print(f"  reportlab        : {'✓' if RL_OK       else '✗  pip install reportlab  (اختياري)'}")
    print("═"*55)
    if not ARABIC_FIX:
        print("  ⚠  تحذير: النص العربي من PDF قد يظهر معكوساً")
        print("     الحل: pip install python-bidi arabic-reshaper")
        print("═"*55)
    App().mainloop()